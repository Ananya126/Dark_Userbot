...
async def yardim(event):
    ...
    if tgbotusername is not None:
        results = await event.client.inline_query(tgbotusername, "@Dark_public_bot")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of 𝙳𝙰𝚁𝙺𝙱𝙾𝚃\n\n"
        )
        hellcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the 𝙳𝙰𝚁𝙺𝙱𝙾𝚃 are [here]({url})"
